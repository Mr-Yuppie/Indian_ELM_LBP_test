...
if strcmp(mappingtype,'u2') %Uniform 2 均匀
  newMax = samples*(samples-1) + 3; % LBP特征维度（bins数目） m（m-1）+3
  for i = 0:2^samples-1
    j = bitset(bitshift(i,1,samples),1,bitget(i,samples)); %rotate left左旋转
...
if strcmp(mappingtype,'ri') %Rotation invariant 旋转不变
